#include "threading/lockfree_mpsc_queue.hpp"
...
TEST(threading_lockfree_mpsc_queue, size) {
  static_assert(
      sizeof(core::threading::lockfree_mpsc_queue<int, std::size_t, 10>) ==
      256);
  static_assert(
      alignof(core::threading::lockfree_mpsc_queue<int, std::size_t, 10>) ==
      64);
  static_assert(sizeof(core::threading::lockfree_mpsc_queue<int>) == 256);
  static_assert(alignof(core::threading::lockfree_mpsc_queue<int>) == 64);
}

TEST(threading_lockfree_mpsc_queue, capacity) {
  EXPECT_EQ(
      (core::threading::lockfree_mpsc_queue<std::string, std::size_t, 128>()
           .capacity()),
      128);
  EXPECT_EQ(
      (core::threading::lockfree_mpsc_queue<std::string, std::size_t, 64>()
           .capacity()),
      64);
  EXPECT_EQ(core::threading::lockfree_mpsc_queue<std::string>(128).capacity(),
            128);
  EXPECT_EQ(core::threading::lockfree_mpsc_queue<std::string>(64).capacity(),
            64);
}

TEST(threading_lockfree_mpsc_queue, minimal_capacity) {
  EXPECT_ANY_THROW(core::threading::lockfree_mpsc_queue<int> queue(0));
  EXPECT_ANY_THROW(core::threading::lockfree_mpsc_queue<int> queue(1));
  EXPECT_NO_THROW(core::threading::lockfree_mpsc_queue<int> queue(2));
}

TEST(threading_lockfree_mpsc_queue, nonblocking_smoke) {
  int value;
...
TEST(threading_lockfree_mpsc_queue, blocking_smoke) {
  core::threading::lockfree_mpsc_queue<int> queue(2);
...
TEST(threading_lockfree_mpsc_queue, smoke) {
  int value;
  core::threading::lockfree_mpsc_queue<int> queue(2);
...
TEST(threading_lockfree_mpsc_queue, blocking_push) {
  core::threading::lockfree_mpsc_queue<int> queue(2);
...
TEST(threading_lockfree_mpsc_queue, blocking_pop) {
  core::threading::lockfree_mpsc_queue<int> queue(2);
...
TEST(threading_lockfree_mpsc_queue, capacity_one) {
  EXPECT_ANY_THROW(core::threading::lockfree_mpsc_queue<int> queue(1));
}

TEST(threading_lockfree_mpsc_queue, queue_destructor) {
  std::shared_ptr<int> value = std::make_shared<int>();
  {
    core::threading::lockfree_mpsc_queue<std::shared_ptr<int>> queue(2);
...
TEST(threading_lockfree_mpsc_queue, item_destructor) {
  std::shared_ptr<int> value = std::make_shared<int>();
  core::threading::lockfree_mpsc_queue<std::shared_ptr<int>> queue(2);
...
TEST(threading_lockfree_mpsc_queue, non_copyable_item_type) {
  std::unique_ptr<int> value;
  core::threading::lockfree_mpsc_queue<std::unique_ptr<int>> queue(2);
...
namespace threading_lockfree_mpsc_queue_allocator {
...
TEST(threading_lockfree_mpsc_queue, allocator) {
  ...
  EXPECT_TRUE(
      threading_lockfree_mpsc_queue_allocator::allocator<entry_t>::is_clean());
  {
    core::threading::lockfree_mpsc_queue<
        value_t, index_t, 2,
        threading_lockfree_mpsc_queue_allocator::allocator<entry_t>>
        queue;
    ...
    EXPECT_FALSE(threading_lockfree_mpsc_queue_allocator::allocator<
                 entry_t>::is_clean());
  }
  EXPECT_TRUE(
      threading_lockfree_mpsc_queue_allocator::allocator<entry_t>::is_clean());
...
class threading_lockfree_mpsc_queue_workload
    : public ::testing::TestWithParam<std::tuple<
          std::size_t /* items_size */, std::size_t /* queue_size */,
          std::size_t /* producers */, std::size_t /* consumers */>> {};

TEST_P(threading_lockfree_mpsc_queue_workload, nonblocking) {
  const auto& [items_size, queue_size, producers, consumers] = GetParam();

  core::threading::lockfree_mpsc_queue<int> queue(queue_size);
...
TEST_P(threading_lockfree_mpsc_queue_workload, blocking) {
  const auto& [items_size, queue_size, producers, consumers] = GetParam();
...
INSTANTIATE_TEST_SUITE_P(
    threading_lockfree_mpsc_queue_workload,
    threading_lockfree_mpsc_queue_workload,
...
TEST(lockfree_mpsc_queue, rollover) {
  core::threading::lockfree_mpsc_queue<int> queue(8);
