...
TEST(threading_lockfree_mpsc_queue, size) {
  static_assert(sizeof(core::threading::lockfree_mpsc_queue<int, 10>) == 256);
  static_assert(alignof(core::threading::lockfree_mpsc_queue<int, 10>) == 64);
...
TEST(threading_lockfree_mpsc_queue, capacity) {
  EXPECT_EQ(
      (core::threading::lockfree_mpsc_queue<std::string, 128>().capacity()),
      128);
  EXPECT_EQ(
      (core::threading::lockfree_mpsc_queue<std::string, 64>().capacity()), 64);
...
TEST(threading_lockfree_mpsc_queue, allocator) {
  ...
  EXPECT_TRUE(threading_lockfree_mpsc_queue_allocator::allocator<
              allocator_value_t>::is_clean());
  {
    core::threading::lockfree_mpsc_queue<
        queue_value_t, 2,
        threading_lockfree_mpsc_queue_allocator::allocator<allocator_value_t>>
        queue;
