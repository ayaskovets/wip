...
TEST(threading_lockfree_spmc_queue, minimal_capacity) {
...
TEST(threading_lockfree_spmc_queue, blocking_push) {
  core::threading::lockfree_spmc_queue<int> queue(2);
...
TEST(threading_lockfree_spmc_queue, blocking_pop) {
  core::threading::lockfree_spmc_queue<int> queue(2);
...
TEST(threading_lockfree_spmc_queue, capacity_one) {
  EXPECT_ANY_THROW(core::threading::lockfree_spmc_queue<int> queue(1));
}

TEST(threading_lockfree_spmc_queue, queue_destructor) {
  std::shared_ptr<int> value = std::make_shared<int>();
  {
    core::threading::lockfree_spmc_queue<std::shared_ptr<int>> queue(2);
...
TEST(threading_lockfree_spmc_queue, item_destructor) {
  std::shared_ptr<int> value = std::make_shared<int>();
  core::threading::lockfree_spmc_queue<std::shared_ptr<int>> queue(2);
...
TEST(threading_lockfree_spmc_queue, non_copyable_item_type) {
  std::unique_ptr<int> value;
  core::threading::lockfree_spmc_queue<std::unique_ptr<int>> queue(2);
...
TEST_P(threading_lockfree_spmc_queue_workload, nonblocking) {
  const auto& [items_size, queue_size, producers, consumers] = GetParam();

  core::threading::lockfree_spmc_queue<int> queue(queue_size);
...
TEST_P(threading_lockfree_spmc_queue_workload, blocking) {
  const auto& [items_size, queue_size, producers, consumers] = GetParam();
...
TEST(lockfree_spmc_queue, rollover) {
  core::threading::lockfree_spmc_queue<int> queue(8);
