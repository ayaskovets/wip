...
void BM_threading_lockless_mpmc_queue_spsc_throughput(benchmark::State& state) {
  ...
  for (const auto _ : state) {
    ...
    core::threading::lockless_mpmc_queue<value_t, core::utils::kRuntimeCapacity,
                                         allocator_t>
        queue(capacity);
    std::latch latch(3);
...
BENCHMARK_TEMPLATE(BM_threading_lockless_mpmc_queue_spsc_throughput, int)
    ->Args({1024, 1048576})
    ->MeasureProcessCPUTime()
    ->UseRealTime()
    ->Unit(benchmark::kMillisecond);
BENCHMARK_TEMPLATE(BM_threading_lockless_mpmc_queue_spsc_throughput,
                   std::shared_ptr<int>)
    ->Args({1024, 1048576})
    ->MeasureProcessCPUTime()
    ->UseRealTime()
...
template <typename ValueConstructor>
void BM_threading_lockless_mpmc_queue_mpmc_throughput(benchmark::State& state) {
  const std::size_t capacity = state.range(0);
  const std::size_t items = state.range(1);
