...
TEST(_TMP__logging, writer) {
  ...
  const _TMP_::logging::writer writer(
      [&buffer]([[maybe_unused]] _TMP_::logging::level level,
                std::string_view message) { buffer = message; });
  const _TMP_::logging::logger logger(level, renderer, writer);
...
TEST(_TMP__logging, renderer) {
  ...
  const auto level = _TMP_::logging::level::kInfo;
  const _TMP_::logging::renderer renderer(
      []([[maybe_unused]] _TMP_::logging::level level, std::string_view fmt,
