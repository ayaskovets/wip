...
void BM_ip4_from_bytes(benchmark::State& state) {
  ...
  for (const auto _ : state) {
    benchmark::DoNotOptimize(ip::address(bytes));
  }
}
...
void BM_ip4_from_string(benchmark::State& state) {
  ...
  for (const auto _ : state) {
    benchmark::DoNotOptimize(ip::address(string));
  }
}
...
void BM_ip6_from_bytes(benchmark::State& state) {
  ...
  for (const auto _ : state) {
    benchmark::DoNotOptimize(ip::address(std::move(bytes)));
  }
}
...
void BM_ip6_from_string(benchmark::State& state) {
  ...
  for (const auto _ : state) {
    benchmark::DoNotOptimize(ip::address(std::move(string)));
  }
}
...
void BM_resolve_localhost(benchmark::State& state) {
  for (const auto _ : state) {
    benchmark::DoNotOptimize(ip::resolve("localhost", ip::protocol::kTcp));
  }
}

void BM_resolve_google(benchmark::State& state) {
  for (const auto _ : state) {
    benchmark::DoNotOptimize(ip::resolve("google.com", ip::protocol::kTcp));
  }
}
